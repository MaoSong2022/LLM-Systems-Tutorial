...
\begin{block}{Value function}
        ...
        \end{equation}
        value function 的具体含义为：\textbf{agent 从当前状态 $s$ 出发，一直遵循当前策略 $\pi$， 最后获取到的 expected discounted return}.
\end{block}
...
\begin{block}{Q function}
    ...
    \end{equation}
    Q function 的具体含义为：\textbf{agent 从当前状态 $s$ 出发，执行动作 $a$, 一直遵循当前策略 $\pi$, 最后获取到的 expected discounted return}.
\end{block}
...
\begin{frame}{Relationship between value function and Q function}
    ...
    \end{proof}
    \begin{block}{Remark}
        value function 是 Q function 的期望，Q function 是 value function 的期望。
    \end{block}
\end{frame}
...
\begin{frame}{Iterative property}
    ...
    \begin{align}
        V^\pi(s) &= \mathbb{E}_{a_0\sim \pi(\cdot\mid s),\ (r_0,s_0)\sim p(\cdot,\cdot\mid s,a_0)}[r_0 + \gamma V^\pi(s_1)\mid s_0=s]\\
    ...
    \begin{proof}
        \begin{align}
            ...
            &= \mathbb{E}_{a_0\sim \pi(\cdot\mid s_0),\ (r_0,s_1)\sim p(\cdot,\cdot\mid s,a_0)}\left[\mathbb{E}^\pi[r_0 + \gamma G_1\mid s_0,a_0,r_0,s_1]\mid s_0=s\right]\\
            &= \mathbb{E}_{a_0\sim \pi(\cdot\mid s_0),\ (r_0,s_1)\sim p(\cdot,\cdot\mid s,a_0)}\left[r_0 + \gamma \mathbb{E}^\pi[G_1\mid s_1]\mid s_0=s\right]\\
            &= \mathbb{E}_{a_0\sim \pi(\cdot\mid s_0),\ (r_0,s_1)\sim p(\cdot,\cdot\mid s,a_0)}\left[r_0 + \gamma V^\pi[s_1)\mid s_0=s\right]
        \end{align}
    \end{proof}
\end{frame}
...
\begin{frame}{Bellman Optimality Equation Theorem--Proof}
    ...
    \begin{proof}
        \begin{align*}
        \mathcal{T}^\pi(V)(s) &=  \mathbb{E}_{a\sim \pi(\cdot\mid s),\ (r,s')\sim p(\cdot,\cdot\mid s,a_0))}\left[r+\gamma V(s')\mid s\right]\leq \mathbb{E}_{a\sim \pi(\cdot\mid s)}\left[\max_{a'} \mathbb{E}_{(r,s')\sim p(\cdot,\cdot\mid s,a')}\left[r+\gamma V(s')\mid s\right]\right]\\
...
\begin{frame}{Bellman Optimality Equation Theorem--Proof}
    ...
    \begin{proof}
        ...
        此时，我们有
        \begin{align}
            V^*(s) &= \mathbb{E}^{\pi^*}\left[r_0+\gamma V^*(s_1)\mid s_0=s\right]\\
            ...
            &= \mathbb{E}^{\pi^*}\left[r_0+\gamma r_1+\gamma^2r_2+\cdots\mid s_0=s\right]= V^{\pi^*}(s)
        \end{align}
        ...
        \end{equation}
        这里第一个等式是 Bellman equation, 第一个不等式是由于下面的 Lemma,最后的极限使用了不动点定理。
        因此我们有 $V^\pi\leq V^*$, 从而 $\pi^*$ 是最优策略，且 $V^*$ 是对应的 optimal value function. 
    \end{proof}
\end{frame}
...
\begin{frame}{Introduction}
    在本节中，我们将介绍如何评估 (evaluate) 一个 policy. 要评估一个 policy $\pi$, 我们需要求出对应的 value function $V^\pi$.

    在上一节中，我们其实已经给出了一种iterative algorithm 来求解value function, 但是，这个算法的问题在于：收敛速度慢，并且state space 和 action space 较大时，计算复杂度较高。

    因此，我们在本节中介绍如何高效的求解/拟合value function.
\end{frame}

\begin{frame}{Monte Carlo Introduction}
    TODO: 介绍 Monte Carlo 方法的基本思想。
    \begin{block}{Monte Carlo}
        Monte Carlo (MC) 方法是一种基于随机采样的方法，用于估计期望值。它通过从样本中估计期望值，从而得到对期望值的估计。
    \end{block}
    \begin{block}{Monte Carlo Estimator}
        \begin{equation}
            \hat{V}^\pi(s) = \frac{1}{N}\sum_{i=1}^{N}R_i
        \end{equation}
    \end{block}
\end{frame}
...
\begin{frame}{Monte Carlo}
    ...
    然后我们使用样本平均来近似期望
    \begin{equation}
        V^\pi(s) = \mathbb{E}^\pi\left[\sum_{t=0}^{T-1}\gamma^tr_t\mid s_0=s\right]\approx \frac{1}{N}\sum_{i=1}^N\sum_{t=0}^{T-1}\gamma^tr_t^{(i)}
    \end{equation}
...
\begin{frame}{MC and SGD}
    \begin{algorithm}[H]
        ...
        \While{not converged}{
            ...
            $T=t$\;
            $g=\left(V_\phi(s_0)- \sum_{t=0}^{T-1}\gamma^tr_t\right)\nabla_\phi V_\phi(s_0)$\;
...
\begin{frame}{Temporal Difference Learning}
    ...
    \begin{equation}
    \min_{\phi} \mathcal{L}(\phi) =\mathbb{E}_{s\sim p_0}\left[\frac12\left(V_\phi(s)-V^\pi(s)\right)^2\right]=\mathbb{E}_{s\sim p_0}^\pi\left[\left(V_\phi(s)- r_0-\gamma V^\pi(s_1)\right)^2\right]
    \end{equation}
    对应的梯度为
    \begin{equation}
