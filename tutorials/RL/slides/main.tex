...
\begin{definition}[Q function]
    ...
    \end{equation}
    Q function 的具体含义为：\textbf{agent 从当前状态 $s$ 出发，先执行动作 $a$, 然后一直遵循当前策略 $\pi$, 最后获取到的 expected return}.
\end{definition}
...
\begin{frame}{Relationship between value function and Q function}
    ...
    \begin{proof}
        \begin{align}
            ...
            &= \sum_{a\in\mathcal{A}}\mathbb{E}\left[G_0\mid s_t=s, a_0=a\right]\pi(a\mid s_0=s)\\
...
\begin{frame}{Bellman Equation (Value function)}
    \begin{theorem}[Bellman Equation]
        ...
        \begin{equation}
        V^\pi(s) = \mathbb{E}_{a\sim \pi(\cdot\mid s),\ (r,s')\sim p(\cdot,\cdot\mid s,a)}^{\pi}\left[r_0+\gamma V^\pi(s')\mid s_0=s\right]
        \end{equation}
...
\begin{frame}{Bellman Equation Theorem--Proof}
    ...
    &=\mathbb{E}_{a\sim\pi(\cdot\mid s), (r_0,s')\sim p(\cdot,\cdot\mid s,a)}\left[\mathbb{E}^\pi\left[r_0+\gamma G_1\mid s,a,r_0,s'\right]\right]\\
    &= \mathbb{E}_{a\sim\pi(\cdot\mid s), (r_0,s')\sim p(\cdot,\cdot\mid s,a)}\left[r_0+\gamma\mathbb{E}^\pi\left[ G_1\mid s,a,r_0,s'\right]\right]\\
    &= \mathbb{E}_{a\sim\pi(\cdot\mid s), (r_0,s')\sim p(\cdot,\cdot\mid s,a)}\left[r_0+\gamma\mathbb{E}^\pi\left[ G_1\mid s'\right]\right]\\
    &=\mathbb{E}_{a\sim\pi(\cdot\mid s'), (r_0,s')\sim p(\cdot,\cdot\mid s,a)}\left[r_0+V^{\pi}(s')\right]
    \end{align}
    \end{proof}
\end{frame}
