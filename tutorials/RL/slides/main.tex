...
\begin{frame}{Bellman Equation Theorem--Proof}
    \small
    \begin{proof}
        由 \eqref{eq:value-function-iterative-property} 我们可以得到 value function 的等式成立。
...
\begin{frame}{Objective of Value based methods}
    ...
    \begin{equation}\label{eq:value-based-methods-objective}
        \min_{\phi} \mathcal{L}(\phi) =\mathbb{E}\left[\frac12\left(V_\phi(s_t)-V^\pi(s_t)\right)^2\right]
    \end{equation}
...
\begin{frame}{Gradient Calculation}
    ...
    \begin{align}
    \nabla_\phi \mathcal{L}(\phi) &= \mathbb{E}\left[\left(V_\phi(s_t)-V^\pi(s_t)\right)\nabla_\phi V_\phi(s_t)\right]\\
    &= \mathbb{E}\left[\left(V_\phi(s_t)- \mathbb{E}^\pi\left[G_t\mid s_t\right]\right)\nabla_\phi V_\phi(s_t)\right]\\
    &=\mathbb{E}\left[\mathbb{E}^\pi\left[\left(V_\phi(s_t)- G_t\right)\nabla_\phi V_\phi(s_t)\mid s_t\right]\right]\\
    &= \mathbb{E}^\pi\left[\left(V_\phi(s_t)- G_t\right)\nabla_\phi V_\phi(s)\mid s_t\right]
    \end{align}
    我们现在可以使用MC方法来估计梯度，即
    \begin{equation}
