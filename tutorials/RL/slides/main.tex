...
\begin{frame}{Relationship between value function and Q function}
    ...
    \begin{proof}
        ...
        \begin{align}
            V^{\pi}(s_t)  &= \mathbb{E}^{\pi}[G_t\mid s_t] \\
            &= \sum_{a_t\in\mathcal{A}}\mathbb{E}^{\pi}\left[G_t\mid s_t, a_t\right]\pi(a_t\mid s_t)\\
            &= \sum_{a_t\in\mathcal{A}}Q^{\pi}(s_t,a_t)\pi(a_t\mid s_t)= \mathbb{E}_{a_t\sim \pi(\cdot\mid s_t)}^{\pi}[Q^{\pi}(s_t,a_t)].
        \end{align}
    \end{proof}
\end{frame}

\begin{frame}{Iterative Property}
    \begin{lemma}[Iterative Property]\label{lemma:iterative-property}
        Value function 和 Q function 存在如下递推关系
        \begin{align}
            V^\pi(s_t) &= \mathbb{E}_{a_t\sim \pi(\cdot\mid s_t),\ (r_t,s_{t+1})\sim p(\cdot,\cdot\mid s_t,a_t)}[r_t + \gamma V^\pi(s_{t+1})\mid s_t] \label{eq:value-function-iterative-property}\\
    ...
    \begin{proof}
        ...
        \end{align}
        这里用到了 Law of Total Expectation (Frame~\ref{law-of-total-expectation}), Q function 的证明类似，我们这里略过。
    \end{proof}
\end{frame}


\begin{frame}{Bellman Equation Theorem (Value function)}
    \begin{block}{}
    前面我们介绍了value function的迭代性质，接下来我们证明满足迭代性质的函数是唯一的。
    \end{block}
...
\begin{frame}{Fixed Point Theorem*}
    \begin{definition}[Fix Point]
        对于函数 $f:\mathbb{R}^n\to\mathbb{R}^n$, 如果存在一个点 $x^*\in\mathbb{R}^n$ 满足
        \begin{equation}
        f(x^*)=x^*
        \end{equation}
...
\begin{frame}{Fixed Point Theorem*}
    \begin{theorem}[Fixed Point Theorem]\label{theorem:fixed-point-theorem}
...
\begin{frame}{Bellman Equation Theorem--Proof}
    \small
    \begin{proof}
        由 Lemma~\ref{lemma:iterative-property} 我们知道value function 满足 Bellman equation.
        ...
        根据不动点定理~\ref{theorem:fixed-point-theorem}，已知 $V^\pi$ 是一个不动点，而不动点唯一，因此我们有 $V=V^\pi$.
    \end{proof}
\end{frame}
...
\begin{frame}{Bellman Equation (Q function)}
    ...
    \end{theorem}
    \begin{proof}
        证明与 value function 的证明类似，我们这里略过。
    \end{proof}
\end{frame}
...
\begin{frame}{Optimal Policy}
    ...
    \end{definition}
    \begin{block}{Remark}
        注意 optimal policy 与状态 $s$ 无关，并且 optimal policy 是不唯一的，后文会证明所有的 optimal policy 对应的 optimal value function 是同一个。
    \end{block}
\end{frame}
    ...
    \begin{proof}
        ...
        \begin{equation}
            \begin{aligned}
                ...
                &\leq \max_{a \in \mathcal{A}} \mathbb{E}_{(r_t, s_{t+1}) \sim p(\cdot \mid s_t, a)} [r_t + \gamma V(s_{t+1})] = \mathcal{T}^* V(s_t)
            \end{aligned}
        \end{equation}
        证毕。
    \end{proof}
...
\begin{frame}{Bellman Optimality Equation Theorem--Proof}
    ...
    \begin{proof}
        ...
        \begin{equation}
        V^\pi=\mathcal{T}^{\pi}V \overset{\text{Lemma~\ref{lemma:bellman-optimality-equation-theorem-proof-1}}}{\le} \mathcal{T}^*V
        ...
        \end{equation}
        即 $V^\pi\leq V^*$, 由于 $V^\pi$ 是任意的，我们有 $V^*=V^{\pi^*}$.
    \end{proof}
\end{frame}
...
\begin{frame}{Why Policy Evaluation?}
    ...
    \begin{block}{Difficulty}
        ...
        但当状态空间较大、动作空间较大时，直接进行精确求解往往计算代价较高。
    \end{block}
    \begin{block}{Core Idea}
...
\begin{frame}{Monte Carlo Introduction}
    ...
    \begin{block}{In RL}
        ...
        \]
        从 $s_0$ 出发按照策略 $\pi$ 采样多条完整轨迹，
        并用这些轨迹 return 的平均值来近似 $V^\pi(s_0)$.
    \end{block}
    \begin{block}{Remark}
        \begin{itemize}
            \item[Pros]: 简单、直观、无需环境模型
            \item[Cons]: 必须等轨迹结束后才能得到一次完整的估计
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Monte Carlo}
    注意到
    \begin{equation}
    V^\pi(s) = \mathbb{E}^\pi\left[R(\tau)\mid s_0\right]
    \end{equation}

    我们可以利用 Monte Carlo (MC) 方法来进行估计，即从 $s_0=s$ 出发，独立随机采样 $N$ 条轨迹

    \begin{equation}
        \{\tau^{(i)}=(s_0^{(i)}, a_0^{(i)},r_0^{(i)},\dots,a_{T-1}^{(i)},r_{T-1}^{(i)}, s^{(i)}_{T})\}, i=1,\dots,N
    \end{equation}
    然后我们使用样本平均来近似期望
    \begin{equation}
        V^\pi(s) = \mathbb{E}^\pi\left[R(\tau)\mid s_0\right]\approx \frac{1}{N}\sum_{i=1}^NR(\tau^{(i)})
    \end{equation}
    这样，我们就得到了一个 $V^\pi(s)$ 的 Monte Carlo estimator.
...
\begin{frame}{Objective of Value based methods}
    当状态空间较大或者连续时，我们可以使用函数逼近的方法来近似 $V^\pi$.

    具体做法就是，我们使用函数 $V_\phi$ 来近似 $V^\pi$, 这里 $\phi$ 是函数的参数，我们使用 least square error 来作为损失函数
    \begin{equation}\label{eq:value-based-methods-objective}
        \min_{\phi} \mathcal{L}(\phi) =\mathbb{E}\left[\frac12\left(V_\phi(s_0)-V^\pi(s_0)\right)^2\right]
    \end{equation}
    然后我们可以使用不同的优化方法来更新参数 $\phi$.

    \footnotetext{这里关于$s$的概率分布需要进一步讨论，请参考 <强化学习的数学原理>}
\end{frame}

\begin{frame}{Gradient Calculation}
    对目标函数~\eqref{eq:value-based-methods-objective} 求导得到
    \begin{align}
    \nabla_\phi \mathcal{L}(\phi) &= \mathbb{E}\left[\left(V_\phi(s_0)-V^\pi(s_0)\right)\nabla_\phi V_\phi(s_0)\right]\\
    &= \mathbb{E}\left[\left(V_\phi(s_0)- \mathbb{E}^\pi\left[G_0\mid s_0\right]\right)\nabla_\phi V_\phi(s_0)\right]\\
    &=\mathbb{E}\left[\mathbb{E}^\pi\left[\left(V_\phi(s_0)- G_0\right)\nabla_\phi V_\phi(s_0)\mid s_0\right]\right]\\
    &= \mathbb{E}^\pi\left[\left(V_\phi(s_0)- G_0\right)\nabla_\phi V_\phi(s_0)\mid s_0\right]
    \end{align}
    我们现在可以使用 MC 方法来估计梯度，即
    \begin{equation}
...
\begin{frame}{Temporal Difference Learning}
    ...
    \begin{equation}
    \min_{\phi} \mathcal{L}(\phi) =\mathbb{E}\left[\frac12\left(V_\phi(s_0)-V^\pi(s_0)\right)^2\right]=\mathbb{E}\left[\left(V_\phi(s_0)- r_0-\gamma V^\pi(s_1)\right)^2\right]
    \end{equation}
    对应的梯度为
    \begin{equation}
    \nabla_\phi \mathcal{L}(\phi) =\mathbb{E}\left[\left(V_\phi(s_0)- r_0-\gamma \boxed{V^\pi(s_1)}\right)\nabla_\phi V_\phi(s_0)\mid s_0\right]
    \end{equation}
    令
    \begin{equation}
    g := \left(V_\phi(s_0)- r_0-\gamma \boxed{V^\pi(s_1)}\right)\nabla_\phi V_\phi(s_0)
    \end{equation}
    可以看到，这里我们使用了 $V^\pi(s_1)$, 而这个值是未知的，因此，一个做法是使用当前的 value function $V_\phi$ 来进行代替，也就是
    \begin{equation}
    \nabla_\phi \mathcal{L}(\phi) \approx \mathbb{E}\left[\left(V_\phi(s)- r_0-\gamma V_\phi(s_1)\right)\nabla_\phi V_\phi(s)\mid s_0\right]
    \end{equation}
\end{frame}
...
\begin{frame}{Temporal Difference Learning--Naive version}
    ...
    \begin{algorithm}[H]\label{algo:td-learning-naive-version}
...
\begin{frame}[fragile]{Temporal Difference Learning--Analysis}
    尽管算法~\ref{algo:td-learning-naive-version} 在数学上是正确的，但是从自动微分的角度来看，是不正确的，比如 Pytorch 对应的实现应该是
    ...
    可以看到，我们实际上求的梯度是
    \begin{equation}
        \nabla_\phi\left[\frac12\left(V_\phi(s_0)-(r_0+\gamma V_\phi(s_1))\right)^2\right]=(V_\phi(s_0)-(r_0+\gamma V_\phi(s_1))(\nabla_\phi V_\phi(s_0)-\nabla_\phi V_\phi(s_1))
    \end{equation}
    这显然与 $g$ 不相等。
\end{frame}
